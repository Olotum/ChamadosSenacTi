...
if num_results > 0:
    ...
    try:
        ...
        usuario_data = [
            ('1', '1', '1', 'tecnico', 'Técnico de Manutenção',
             'ativo', None, None, None),
            ('Técnico Fulano', 'fulano@example.com', 'senha123', 'tecnico',
             'Técnico de Manutenção', 'ativo', None, None, None),
            ('Usuário Ciclano', 'ciclano@example.com', 'senha456',
             'usuario', 'Usuário Comum', 'ativo', None, None, None)
        ]
    ...
    try:
        ...
        equipamento_data = [
            ('Computador', 1, 'Dell', 'Bom estado',
             'ativo', None, '2024-02-21 10:00:00'),
            ('Impressora', 1, 'HP', 'Precisa de manutenção',
             'inativo', 'Quebrada', '2024-02-21 11:00:00'),
            ('Telefone', 2, 'Panasonic', 'Ótimo estado',
             'ativo', None, '2024-02-21 12:00:00')
        ]
    ...
    try:
        ...
        add_chamado = ("INSERT INTO chamados "
                       ...
                       "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")
        chamado_data = [
            (1, 1, 'Computador lento', 1, '2024-02-21 13:00:00',
             1, 'N/A', 'baixa', 'Limpeza realizada'),
            (2, 2, 'Impressora não imprime', 1, '2024-02-21 14:00:00',
             None, 'Toner acabou', 'alta', 'Substituído toner'),
            (2, 2, 'Impressora não imprime', 1, '2024-02-21 14:00:00',
             None, 'Toner acabou', 'alta', 'Substituído toner'),
            (2, 3, 'Telefone com chiado', 2, '2024-02-21 15:00:00', None,
             'Problema de linha', 'média', 'Verificada linha telefônica')
        ]
    ...
    if request.method == 'POST':
        if request.form['filtro'] == "fechados":
            if session.get('perfil') == 'tecnico':
                cursor.execute(
                    'Select * from chamados WHERE data_fechamento IS NOT NULL ORDER BY data_fechamento DESC')
        ...
        if request.form['filtro'] == "abertos":
            if session.get('perfil') == 'tecnico':
                if session.get('perfil') == 'tecnico':
                    cursor.execute(
                        'Select * from chamados WHERE data_fechamento IS NULL')
                    chamados = cursor.fetchall()
            else:
        ...
        if request.form['filtro'] == "fechamento_crec":
            if session.get('perfil') == 'tecnico':
                cursor.execute(
                    'Select * from chamados WHERE data_fechamento IS NOT NULL ORDER BY data_fechamento')
                chamados = cursor.fetchall()
            else:
        ...
        if request.form['filtro'] == "abertura_crec":
            if session.get('perfil') == 'tecnico':
                cursor.execute(
                    'Select * from chamados WHERE data_fechamento IS NULL ORDER BY data_abertura')
                chamados = cursor.fetchall()
            else:
        ...
        if request.form['filtro'] == "abertura_desc":
            if session.get('perfil') == 'tecnico':
                cursor.execute(
                    'Select * from chamados WHERE data_fechamento IS NULL ORDER BY data_abertura DESC')
                chamados = cursor.fetchall()
            else:
...
def status_usuario(id):
    ...
    try:
        ...
        if usuario[6] == "ativo":
            cursor.execute(
                'UPDATE usuarios SET status = %s WHERE id = %s', ('inativo', id))
            cursor.execute(
                'INSERT INTO status (id_usuario, status, date_modificacao) VALUES (%s, %s, NOW())', (id, 'inativo'))
        else:
            cursor.execute(
                'UPDATE usuarios SET status = %s WHERE id = %s', ('ativo', id))
            cursor.execute(
                'INSERT INTO status (id_usuario, status, date_modificacao) VALUES (%s, %s, NOW())', (id, 'ativo'))
        cursor.close()
        cnx.commit()
...
def status_equipamento(id):
    ...
    if request.method == 'POST':
        ...
        if equipamento[5] == "ativo":
            ...
            cursor.execute('UPDATE equipamentos SET status = %s, conservacao = %s, justificativa = %s WHERE id = %s',
                           ('inativo', conservacao, justificativa, id))
            cursor.execute(
                'INSERT INTO status (id_equipamento, status, date_modificacao) VALUES (%s, %s, NOW())', (id, 'inativo'))
            cursor.close()
            cnx.commit()
            return redirect(url_for('pagina_equipamentos'))
            ...
            conservacao = request.form.get('conservacao')
            cursor.execute('UPDATE equipamentos SET status = %s, conservacao = %s, justificativa = %s WHERE id = %s',
                           ('ativo', conservacao, None, id))
            cursor.execute(
                'INSERT INTO status (id_equipamento, status, date_modificacao) VALUES (%s, %s, NOW())', (id, 'ativo'))
            cursor.close()
            cnx.commit()
            return redirect(url_for('pagina_equipamentos'))
    else:
...
def atualizarchamado(id):
    ...
    if request.method == 'POST':
        try:
            ...
            cursor = cnx.cursor()
            sql = 'UPDATE chamados SET id_equipamento = %s, urgencia = %s, observacao = %s WHERE id = %s;'
            values = (id_equipamento, urgencia, observacao, id)
            cursor.execute(sql, values)
            cnx.commit()
            cursor.close()
            cnx.close()
